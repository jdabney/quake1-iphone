...
/* All the ESQUAKE stuff is substituting the old vertex calls for
   array based calls.  Maybe it's faster to upload the arrays as VBOs, 
   but I don't think it makes a different on iPhone. kja */
...
cvar_t	gl_texsort = {"gl_texsort","0"};
...
void GL_DrawAliasFrame (aliashdr_t *paliashdr, int posenum)
{
	...
	static GLfloat vertArray[MAXALIASVERTS*(2+4+3)];
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glTexCoordPointer(2, GL_FLOAT, sizeof(GLfloat)*(2+4+3), vertArray);
	glEnableClientState(GL_COLOR_ARRAY);
	glColorPointer(4, GL_FLOAT, sizeof(GLfloat)*(2+4+3), vertArray+2);
	glEnableClientState(GL_VERTEX_ARRAY);
	glBindBuffer(GL_ARRAY_BUFFER, paliashdr->vertexBufferName);
	glVertexPointer(3, GL_FLOAT, sizeof(GLfloat)*(2+4+3), vertArray+2+4);
...
void GL_DrawAliasFrame (aliashdr_t *paliashdr, int posenum)
{
	...
	while (1)
	{
		...
		float texCoords[2*count];
		float * texCoord = texCoords;
		float colors[4*count];
		float * color = colors;
		GLfloat vertArray[3*count];
		GLfloat * pVert = vertArray;
		const GLsizei arraySize = count;
		do
		{
			*(texCoord++) = ((float *)order)[0];
			*(texCoord++) = ((float *)order)[1];
			order += 2;
		...
		glVertexPointer(3, GL_FLOAT, sizeof(GLfloat)*3, vertArray);
		glEnableClientState(GL_TEXTURE_COORD_ARRAY);
		glEnableClientState(GL_COLOR_ARRAY);
		glEnableClientState(GL_VERTEX_ARRAY);
		glDrawArrays(mode, 0, arraySize);
